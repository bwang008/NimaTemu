...
def test_new_fixed_values():
    """Test that the new fixed column values were applied correctly."""
    
    try:
        # Load the output file
        output_df = pd.read_excel('output/temu_upload_generated_with_fixed_values.xlsx', sheet_name='Template', header=1)
        
        print("TESTING NEW FIXED COLUMN VALUES")
        print("=" * 50)
        
        # Check the new fixed values that should have been set
        ...
        print("Checking new fixed values in output file:")
        print("-" * 40)
        ...
        print("-" * 40)
        sample_columns = ['Product Name', 'Update or Add', 'Shipping Template', 'Size', 'Category']
        ...
        # Summary of all fixed values
        print(f"\nALL FIXED VALUES SUMMARY:")
        print("-" * 40)
        all_fixed_values = {
            'Category': '29153',
        ...
        for column, value in all_fixed_values.items():
            if column in output_df.columns:
                print(f"✓ {column} = '{value}'")
            else:
                print(f"✗ {column} = '{value}' (column not found)")
        
    except Exception as e:
        print(f"Error testing new fixed values: {e}")

if __name__ == "__main__":
    test_new_fixed_values() 